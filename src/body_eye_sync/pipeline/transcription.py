...
Whisper is run once over the whole recording rather than over each speech turn
separately: it transcribes far better with surrounding context, and cutting the
audio at turn boundaries would clip words in half. Its word timings are then
assigned to the turns diarization found, which is what attributes the text to a
speaker. This is the same order of operations whisperX uses.
"""
...
#: Default Whisper model. ``large-v3-turbo`` is the best quality that still runs
#: at a sensible speed on CPU; the smaller models trade accuracy for speed.
