"""Audio processing tab: transcribe one input's audio.
...
input shares, and its "Run" button transcribes in a background thread. Whisper
segments are listed as they arrive; they become persistent results only once the
run finishes successfully.

This is a per-input stage, so it says what was said and not who said it: one
recording cannot tell the voices apart. Speakers are settled afterwards, across
all of the experiment's recordings at once.
"""
...
class AudioProcessingTab(BaseTab):
    ...
    def __init__(self, experiment: Experiment) -> None:
        ...
        #: Word count in the provisional transcript shown during a run. The
        #: completed count comes from ``Speech.words`` instead.
    ...
    def _set_transcript_row(
        ...
        values = [_time_text(start), _time_text(end), str(text)]
    ...
    def _update_step_availability(self) -> None:
        """Enable the "Run" buttons once the shown input has audio to run on.
        ...
        self.pipeline_editor.set_run_enabled(TranscriptionStep, has_audio)
    ...
    def _start_step(self, step_type: type) -> None:
        ...
        # Discards that step's previous results.
    ...
    def _begin_run(self) -> None:
        ...
        # The new run has already cleared its persistent result. Keep the table
        # provisional too, rather than showing the previous transcript until
        # Whisper produces its first replacement segment.
        self.transcript_table.clearContents()
