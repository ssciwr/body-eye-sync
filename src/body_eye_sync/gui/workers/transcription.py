...
class TranscriptionWorker(BaseWorker):
    ...
    recording it has reached. The segments are collapsed into the transcript and
    its word timings once the run finishes; a cancelled/failed pass leaves the
    recording without one. The transcription arguments come from ``step``.
    """
    ...
    def _discard(self) -> None:
        # Drops the partial transcript: half a recording's worth of text is
        # not a result, so the recording goes back to having none.
