...
#: The steps of a :class:`~body_eye_sync.experiment.config.SpeechPipeline`, in run
#: order. Diarization is required; transcription is attributed to the speech turns
#: it finds, so it is opt-in.
SPEECH_STEPS: list[StepEntry] = [
    ("diarization", DiarizationStep, "Diarization", False),
    ("transcription", TranscriptionStep, "Transcription", True),
]
