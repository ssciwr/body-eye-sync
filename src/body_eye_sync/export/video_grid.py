...
def _validate_timeline(data: Video | Audio) -> None:
    """Reject runtime timeline edits that cannot describe media timing."""
    timeline = data.timeline
    if not math.isfinite(timeline.offset):
        raise ValueError(f"input {data.id!r} has a non-finite time offset")
    for shift in timeline.shifts:
        if not math.isfinite(shift.at):
            raise ValueError(f"input {data.id!r} has a non-finite shift time")
        if not math.isfinite(shift.seconds) or shift.seconds <= 0:
            raise ValueError(f"input {data.id!r} has an invalid shift duration")
...
def _observed_spans(timeline: Timeline, duration: float) -> list[ObservedSpan]:
    ...
    shifts = sorted(timeline.shifts, key=lambda shift: shift.at)
    for shift in shifts:
        at = float(shift.at)
        ...
        spans.append(
            ObservedSpan(
                ...
                experiment_end=experiment_start + at - cursor,
            )
        )
        cursor = at
    ...
    spans.append(
        ObservedSpan(
            ...
            local_end=duration,
            experiment_start=experiment_start,
            experiment_end=experiment_start + duration - cursor,
        )
    )
    return spans
...
def construct_video_grid(
    ...
    *,
    ...
    video_spans = [
        _observed_spans(source.data.timeline, source.media.video.duration)
        for source in video_sources
    ]
    audio_sources = [source for source in sources if source.media.audio is not None]
    audio_spans = [
        _observed_spans(source.data.timeline, source.media.audio.duration)
