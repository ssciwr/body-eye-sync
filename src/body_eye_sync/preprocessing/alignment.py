"""Calculate the time offsets that put every input on one shared timeline."""
...
# How many cepstral coefficients to keep
SPECTRAL_COEFFICIENTS = 26

# Settings for the landmark fingerprint from doi.org/10.1007/s12193-015-0196-1
...
# The smallest step in seconds in the measured offset worth calling lost content
DEFAULT_MIN_SHIFT = 0.05

# Fractions of min_shift needed for a ramp to be interpreted as a loss rather than drift
_RAMP_FRACTION = 0.75
_RAMP_INTERVALS = 3

# How far above the rest of the correlation curve a peak must be to be used
MIN_QUALITY = 5.0
...
class DriftPoint:
    ...
    # experiment time at the centre of the window
    time: float
    offset: float
    quality: float
...
def spectral_features(
    ...
    The log-mel spectrogram comes from the extractor faster-whisper ships.
    """
...
def drift_curve(
    ...
    *,
    ...
    ``step`` defaults to half the window, which is the spacing the rest of the
    fit is calibrated for. A loss reaches the curve as a ramp about a window
    wide rather than a clean step, and :func:`detect_shifts` collects that ramp
    back into one loss over twice the point spacing; at half-window steps those
    two are the same width. Stepping finer also stops neighbouring windows
    being independent observations, which ``plateau`` and
    :func:`estimate_time_scale` both count in windows.

    ``min_quality`` overrides the lock threshold the method carries, for
    recordings whose windows are too quiet or too sparse to reach it.
    """
...
def estimate_time_scale(
    points: list[DriftPoint], *, min_shift: float = DEFAULT_MIN_SHIFT
) -> float:
    ...
    ``min_shift`` is the same threshold :func:`detect_shifts` uses, and has to
    be: what this leaves out as a loss is exactly what that goes on to find, so
    a step below one but above the other would be claimed by neither.
    """
    if len(points) < 2:
        return 1.0
    ...
    valid &= np.abs(offset_delta) < min_shift
    ...
    ramp = _RAMP_FRACTION * min_shift
    for start in range(len(offset_delta)):
        end = min(start + _RAMP_INTERVALS, len(offset_delta))
        if abs(offsets[end] - offsets[start]) >= ramp:
...
def detect_shifts(
    ...
    *,
    ...
    ``min_shift`` is the smallest step worth calling a loss; see
    :data:`DEFAULT_MIN_SHIFT` for what the default trades off.
    ...
    if time_scale is None:
        time_scale = estimate_time_scale(points, min_shift=min_shift)
...
def fit_timeline(
    ...
    *,
    ...
    if time_scale is None:
        scale = estimate_time_scale(points, min_shift=min_shift)
    else:
        scale = time_scale
