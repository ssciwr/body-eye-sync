# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

- support importing Tobii Pro Glasses 2 and Glasses 3 recording folders, including gaze and head movement data [#61](https://github.com/ssciwr/body-eye-sync/pull/61)
- add a layout chooser to the combined video export tab [#58](https://github.com/ssciwr/body-eye-sync/pull/58)
- replace Timing correction tab with Clock rate tab and take gaps into account when decoding audio [#48](https://github.com/ssciwr/body-eye-sync/pull/48)
- add speech transcription, attribution, ELAN export, and GUI audio processing / speech post-processing tabs [#44](https://github.com/ssciwr/body-eye-sync/pull/44)
- add synchronised video export functionality and GUI export tab [#43](https://github.com/ssciwr/body-eye-sync/pull/43)
- add automatic recording alignment and Timing correction tab to detect and fix clock drift and gaps [#38](https://github.com/ssciwr/body-eye-sync/pull/38)
- add Alignment tab to align recordings [#41](https://github.com/ssciwr/body-eye-sync/pull/41)
- split the GUI into a tab per stage, with an Input files tab for managing an experiment's inputs [#34](https://github.com/ssciwr/body-eye-sync/pull/34)
- add `GlassesVideo`, `FixedVideo` and `Audio` input types [#31](https://github.com/ssciwr/body-eye-sync/pull/31)

## [0.0.4] - 2026-07-03

- First PyPI pre-release.
