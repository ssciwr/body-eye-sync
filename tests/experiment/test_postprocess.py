...
def test_each_turn_goes_to_the_wearer_who_was_loudest(experiment):
    ...
    assert experiment.speech_turns.data["text"].tolist() == [
        "p1 speaking",
        "p2 speaking",
    ]
    assert experiment.speech_turns.speakers == ["p1", "p2"]


def test_experiment_post_processing_settings_control_attribution(experiment):
    experiment.pipeline.speech_post_processing.live_above_floor_db = 100.0

    attribute_experiment_speech(experiment)

    assert experiment.speech_turns.data.empty
...
def test_a_fixed_camera_is_not_attributed_to_anyone(experiment, tmp_path):
    ...
    assert "room" not in experiment.speech_turns.data["speaker"].tolist()
...
def test_attribution_survives_a_recording_that_started_late(experiment):
    ...
    experiment.glasses_videos[1].timeline.offset = -2.0
