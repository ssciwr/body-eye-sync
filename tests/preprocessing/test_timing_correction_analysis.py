...
def test_analysis_forwards_the_measurement_settings(monkeypatch):
    ...
    def curve(reference, other, offset, **kwargs):
        seen.update(kwargs)
        return [DriftPoint(10.0, 0.0, 9.0)]

    monkeypatch.setattr(
        "body_eye_sync.preprocessing.timing_correction.drift_curve", curve
    )
    ...
    # Left as None so drift_curve derives it from the window.
    assert seen["step"] is None


@pytest.mark.parametrize(
    ("fit_drift", "expected_time_scale"),
    [(False, 1.0), (True, None)],
)
def test_fit_drift_decides_whether_a_rate_is_fitted_at_all(
    monkeypatch, fit_drift, expected_time_scale
):
    """Off, the fit is pinned to a clock that kept time and every step is a gap."""
    features = {"a": np.zeros((5_000, 1)), "b": np.zeros((5_000, 1))}
    monkeypatch.setattr(
        "body_eye_sync.preprocessing.timing_correction.SPECTRAL",
        spectral_method(lambda path: features[Path(path).stem]),
    ...
    monkeypatch.setattr(
        "body_eye_sync.preprocessing.timing_correction.drift_curve",
        lambda *args, **kwargs: [DriftPoint(10.0, 0.0, 9.0)],
    )
    seen = {}

    def fit(points, **kwargs):
        seen.update(kwargs)
        return TimelineFit(0.0, 1.0)
    ...
    assert seen["time_scale"] == expected_time_scale
