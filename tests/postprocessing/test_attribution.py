...
def _timelines(**overrides):
    ...
    timelines = {name: Timeline() for name in ("a", "b", "c")}
    timelines.update({k: v for k, v in overrides.items() if v is not None})
    return timelines
...
def test_each_segment_goes_to_the_wearer_who_was_loudest(two_speakers):
    ...
    turns = attribute_segments(transcripts, levels, _timelines())
...
def test_word_timings_rescue_a_phrase_from_a_long_mixed_segment():
    times = np.arange(0.0, 6.0, HOP_SECONDS)
    above_floor = np.zeros((2, times.size))
    above_floor[0, times < 1.2] = 30.0
    above_floor[1, times >= 2.0] = 30.0
    levels = Levels(
        ["a", "b"],
        times,
        above_floor,
    ...
    transcripts = {
        "a": _transcript((0.0, 6.0, "Aber Nudeln echt nice, somebody else continues"))
    }
    words = {
        "a": _words(
            (0.0, 0.3, "Aber"),
    ...
    turns = attribute_segments(transcripts, levels, _timelines(), words=words)

    assert turns["text"].tolist() == ["Aber Nudeln echt nice,"]
    assert turns["start"].tolist() == [0.0]
    assert turns["end"].tolist() == [1.2]
    assert turns["source_segment_id"].tolist() == [0]

    unsplit = attribute_segments(
        transcripts,
        levels,
        _timelines(),
        words=words,
        settings=SETTINGS.model_copy(update={"split_gap_seconds": 10.0}),
    )
    assert unsplit.empty


def test_substantial_comma_delimited_clauses_are_split():
    ...
    words = _words(
        *((index * 0.3, (index + 1) * 0.3, token) for index, token in enumerate(tokens))
    )

    split = _split_attribution_segments(transcript, words, SETTINGS)

    assert split["text"].tolist() == [
        "one two three four,",
...
def test_comma_does_not_split_off_a_short_clause():
    transcript = _transcript((0.0, 2.0, "Yes, one two three four five."))
    words = _words(
        (0.0, 0.3, "Yes,"),
        (0.3, 0.6, "one"),
        (0.6, 0.9, "two"),
    ...
    assert split.to_dict("records") == transcript.to_dict("records")


def test_shifted_word_timings_do_not_hide_duplicate_split_clauses():
    stronger = _Segment(
        "a", 0, 1.0, 2.2, "weil dann sind die schon überbacken.", 0.9, 1.0
    )
    quieter = _Segment(
    ...
    assert _is_bleed(quieter, [stronger], spoken, SETTINGS)


def test_a_generic_two_word_match_does_not_remove_a_different_clause():
    first = _Segment("a", 0, 1.0, 2.2, "Du meinst es nicht so in,", 0.9, 1.0)
    second = _Segment(
    ...
    assert not _is_bleed(second, [first], spoken, SETTINGS)


def test_fuzzy_characters_match_noisy_whisper_versions_with_shifted_timings():
    stronger = _Segment(
        "a", 0, 82.504, 84.184, "Jaja, ich mache voll oft beides.", 0.79, 0.85
    )
    quieter = _Segment(
    ...
    assert _is_bleed(
        ...
        [stronger],
        spoken,
        SETTINGS.model_copy(update={"fuzzy_agreement": 0.75}),
    )
    assert not _is_bleed(
        quieter,
        [stronger],
        spoken,
        SETTINGS.model_copy(update={"fuzzy_agreement": 0.8}),
    )
...
def test_transcripts_are_placed_on_the_experiment_clock(tmp_path):
    ...
    turns = attribute_segments(
        {"b": _transcript((8.0, 11.0, "mine"))}, levels, timelines
    )
...
def test_a_second_speaker_is_kept_when_they_say_something_else(talking_over):
    levels = measure_levels(talking_over, _timelines())
    transcripts = {
...
def test_how_much_agreement_counts_as_bleed_can_be_chosen(talking_over):
    ...
    lenient = attribute_segments(
        transcripts,
        levels,
        _timelines(),
        settings=SETTINGS.model_copy(update={"fuzzy_agreement": 0.9}),
    )
    assert sorted(lenient["speaker"]) == ["a", "b"]

    strict = attribute_segments(
        transcripts,
        levels,
        _timelines(),
        settings=SETTINGS.model_copy(update={"fuzzy_agreement": 0.6}),
    )
...
def test_two_copies_that_both_lost_are_still_kept_once():
    ...
    turns = attribute_segments(transcripts, levels, _timelines(a=None, b=None, c=None))
