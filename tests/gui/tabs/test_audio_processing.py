...
def experiment(data_dir):
    """An experiment with an audio input and a video input."""
    return Experiment(
        ExperimentConfig(
            fixed_videos=[
...
def test_without_inputs_nothing_can_be_run(empty_tab):
    ...
    assert empty_tab.transcription_checkbox.isChecked()
...
def test_every_input_is_listed(tab):
    assert [
    ...
    assert tab.transcription_checkbox.isChecked()
    assert _run_button(tab, TranscriptionStep).isEnabled()
    assert tab.summary_label.text() == "No transcript yet; run transcription."


def test_a_video_input_is_transcribed_from_its_own_file(
    qtbot, tab, data_dir, fast_speech
):
    ...
    assert fast_speech["audio_path"] == data_dir / "three-people-talking.mp4"
...
def test_run_transcription_populates_the_results_and_the_table(qtbot, tab):
    ...
    assert tab.transcript_table.columnCount() == 3
...
def test_run_all_selects_and_transcribes_every_recording(qtbot, tab, monkeypatch):
    calls = []
    continue_runs = [threading.Event(), threading.Event()]

    def streaming_transcribe(audio_path, **kwargs):
        run = len(calls)
        calls.append((audio_path.name, kwargs))
        yield TRANSCRIPT[0]
        continue_runs[run].wait(timeout=5)
        yield from TRANSCRIPT[1:]
    ...
    try:
        qtbot.waitUntil(
            lambda: len(calls) == 1 and tab.transcript_table.rowCount() == 1,
            timeout=5000,
        )
        assert tab.input_selector.currentText() == "room (fixed)"

        continue_runs[0].set()
        qtbot.waitUntil(
            lambda: (
                len(calls) == 2
                and tab.input_selector.currentText() == "mic1 (audio)"
                and tab.transcript_table.rowCount() == 1
            ),
            timeout=5000,
        )
        assert tab.is_busy()
    finally:
        for event in continue_runs:
            event.set()

    qtbot.waitUntil(lambda: not tab.is_busy(), timeout=10000)

    assert [name for name, _ in calls] == [
        "three-people-talking.mp4",
        "three-people-conversation.opus",
    ]
    assert [kwargs["model_name"] for _, kwargs in calls] == ["tiny", "tiny"]
    assert all(data.speech.words is not None for data in tab._recordings)
    assert tab.input_selector.currentText() == "mic1 (audio)"
    assert tab._pending_inputs == []
...
def test_playback_highlights_the_segment_at_the_current_time(qtbot, experiment):
    experiment.audio[0].speech.set_data(_segments())
    tab = AudioProcessingTab(experiment)
    qtbot.addWidget(tab)
    tab.input_selector.setCurrentIndex(1)
...
def test_double_clicking_a_segment_seeks_and_starts_playback(
    ...
    tab.input_selector.setCurrentIndex(1)
    calls = []
    monkeypatch.setattr(tab.audio_player, "seek", lambda seconds: calls.append(seconds))
    monkeypatch.setattr(tab.audio_player, "play", lambda: calls.append("play"))

    tab.transcript_table.cellDoubleClicked.emit(1, 2)

    assert calls == [2.5, "play"]
...
def test_editing_the_pipeline_updates_the_experiment(tab):
    assert isinstance(tab.experiment.pipeline.speech.transcription, TranscriptionStep)
...
def test_transcription_can_be_enabled_and_disabled(qtbot, data_dir):
    ...
    assert not tab.transcription_checkbox.isChecked()
    assert not tab.pipeline_editor.isEnabled()
    assert not _run_button(tab, TranscriptionStep).isEnabled()

    tab.transcription_checkbox.click()

    assert isinstance(experiment.pipeline.speech, SpeechPipeline)
    assert tab.pipeline_editor.isEnabled()
...
def test_invalid_transcription_settings_abort_the_run(tab, monkeypatch, fast_speech):
    shown = {}
    monkeypatch.setattr(
...
def test_invalid_transcription_settings_abort_run_all(tab, monkeypatch, fast_speech):
    def invalid(self, step_type):
        raise ValueError("bad settings")

    monkeypatch.setattr(
        "body_eye_sync.gui.widgets.pipeline_editor.PipelineEditor.config_for",
        invalid,
    )
    monkeypatch.setattr(
        "body_eye_sync.gui.tabs.audio_processing.QMessageBox.critical",
...
def test_the_tab_is_locked_and_reports_progress_during_transcription(qtbot, tab):
    busy = []
    progress = []
...
def test_finishing_transcription_reports_it(qtbot, tab):
    messages = []
...
def test_failed_transcription_is_reported(qtbot, tab, monkeypatch):
...
def test_shutdown_cancels_running_transcription(tab):
    tab._start_transcription()
