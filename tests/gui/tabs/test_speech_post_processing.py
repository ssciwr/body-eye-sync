            ...
            glasses_videos=[
                ...
                GlassesVideoInput(
                    ...
                    timeline=TimelineConfig(
                        offset=0.001  # aligned, even if barely
                    ),
                ),
            ]
        ),
...
def test_post_processing_settings_are_shown_and_update_the_experiment(tab):
    splitting = tab.splitting_form._widgets
    attribution = tab.attribution_form._widgets

    assert tab.splitting_group.title() == "Splitting"
    assert tab.attribution_group.title() == "Attribution"
    assert splitting["split_gap_seconds"].value() == pytest.approx(0.75)
    assert splitting["split_on_sentence_end"].isChecked()
    assert splitting["split_on_comma"].isChecked()
    assert splitting["minimum_clause_words"].value() == 4
    assert splitting["minimum_clause_seconds"].value() == pytest.approx(0.5)
    assert attribution["floor_percentile"].value() == pytest.approx(10.0)
    assert attribution["live_above_floor_db"].value() == pytest.approx(10.0)
    assert attribution["fuzzy_agreement"].value() == pytest.approx(0.6)
    splitting["split_gap_seconds"].setValue(1.25)
    attribution["ownership_share"].setValue(0.6)

    settings = tab.experiment.pipeline.speech_post_processing
    assert settings.split_gap_seconds == pytest.approx(1.25)
    assert settings.ownership_share == pytest.approx(0.6)


def test_visible_threshold_is_used_by_the_run(qtbot, tab):
    tab.attribution_form._widgets["live_above_floor_db"].setValue(100.0)

    _attribute(qtbot, tab)

    assert tab.turns_table.rowCount() == 0
    assert tab.summary_label.text() == "No speech was attributed to anyone."
...
def test_stored_turns_are_shown_without_running_anything(qtbot, experiment):
    ...
    row_color = tab.turns_table.item(0, 3).background().color()
    assert row_color.name() == tab.audio_player.color_for("p1").name()
    assert row_color.alpha() < 255


def test_double_clicking_a_turn_seeks_and_starts_shared_audio(
    ...
    qtbot.addWidget(tab)
    calls = []
    monkeypatch.setattr(tab.audio_player, "seek", lambda at: calls.append(at))
    monkeypatch.setattr(tab.audio_player, "play", lambda: calls.append("play"))

    tab.turns_table.cellDoubleClicked.emit(0, 3)

    assert calls == [1.25, "play"]


def test_playback_highlights_all_turns_at_the_current_time(qtbot, experiment):
    experiment.speech_turns.set_data(
        pd.DataFrame(
            {
                "turn_id": [0, 1],
