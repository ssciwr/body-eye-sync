...
def test_export_passes_selection_and_merged_audio_to_backend(
    ...
    elan_calls = []
    monkeypatch.setattr(
        "body_eye_sync.gui.tabs.data_export.export_elan",
        lambda experiment, result, **kwargs: elan_calls.append(
            (experiment, result, kwargs)
        ),
    )
    busy = []
    progress = []
    ...
    assert elan_calls == [
        (
            tab.experiment,
            video_result,
            {"input_ids": ["room", "microphone"], "overwrite": True},
        )
    ]
    assert (50, 100, "Exporting combined video…") in progress
    assert progress[-1] == (100, 100, "Exporting combined video…")
    assert messages == [
        f"Exported combined video to {output} and ELAN annotations to "
        f"{output.with_suffix('.eaf')}"
    ]
