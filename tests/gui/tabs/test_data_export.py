...
def _result(path) -> VideoGridResult:
    """What a finished export reports back, for a stubbed video step."""
...
def _run_export(qtbot, tab, tmp_path, monkeypatch, output_name="chosen.mp4"):
    """Run an export whose video step is stubbed out.

    Returns the output path and the status messages the tab reported.
    """
...
def test_annotations_are_written_beside_the_video(qtbot, tab, tmp_path, monkeypatch):
    ...
    output, messages = _run_export(qtbot, tab, tmp_path, monkeypatch)

    annotations = output.with_suffix(".eaf")
    assert annotations.exists()
    assert annotations.read_text().count("<TIER ") == 2
    assert "wrote speech annotations to chosen.eaf" in messages[-1]
