...
def test_observed_spans_apply_offset_and_unsorted_losses():
    timeline = Timeline(offset=10.0)
    timeline.shifts = [
    ...
    spans = _observed_spans(timeline, 4.0)
...
def test_observed_spans_reject_empty_or_unknown_duration():
    assert _observed_spans(Timeline(), 0.0) == []
    assert _observed_spans(Timeline(), float("nan")) == []
...
def test_construct_video_grid_synchronizes_25_and_50_fps_video_and_audio(tmp_path):
    ...
    assert result.experiment_end == pytest.approx(3.0, abs=0.03)
