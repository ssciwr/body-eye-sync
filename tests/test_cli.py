...
@pytest.mark.parametrize("min_face_frames", [30, 31])
def test_cli_clusters_cached_glasses_results_and_saves_identities(
    tmp_path, min_face_frames
):
    ...
    experiment.pipeline.cluster_post_processing.min_face_frames = min_face_frames
    ...
    expected = pd.DataFrame(
        [
            ("a", 1, "b" if min_face_frames <= 30 else None),
            ("b", 1, "a" if min_face_frames <= 30 else None),
        ],
